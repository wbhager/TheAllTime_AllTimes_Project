...
7/8/2024: Finalizd my point delegation system, changed all outfield
positions to OF to keep consistency, fixed the bug in my team
matchup function that made it so that the team with more points always
wins and that there are no unforeseen edge cases, made new variables
that consist of each franchise's total points that will be used in the
tournament, compared point totals across the best teams and the worst
teams in orer to see what the spread of the data looked like, tested to
see that my variables worked with my edited function
(2 hours)

7/9/2024: Grinded on the tourney, decided to split it up into
two different kinds since I realized that if I only did the tourney
in the classic style, it would be unlikely for me to produce one 
of the top teams to be the best team of all time. Seeded
the teams, theorized on how I would set different seeds to play
each other, and then finished setting up the first round, second round,
and ALDS without any bugs.
(4 hours)

7/11/2024: Finished the classic style tourney, tested ways on how to 
return only the team name as the winner instead of all of the winning
team's informaiton, wrote some additional comments and removed redundant
code to make everything cleaner
(1.5 hours)

7/12/2024: Began the second tourney style, got stuck on a very minor bug
regarding indexing
(1 hour)

7/14/2024: Finished the tourney by fixing the bug and adding the NL information.
Main objective is complete! Job is not finished because there is a LOT more
that I can do, such as editing my team assigning process to be like the second
method I came up with and updating every instance where that can be changed,
edit the tourneys WAY down into the method I used to get the world series winner
from the gauntlet (disregard the seeds after they are first used and only use the
winning team's data, basically saving a bajillion if statements), fixing the pirates
but from the first tourney, editing my point delegation system further to reduce 
bias, adding the recent nationals players because I am biased and want to see them
represented, doing the eras side quest, and other things. On to the second project!
(0.5 hours)

Total Days Worked To Complete Project: 21
Total Hours Worked: 46.5 Hours



7/?/2024: 

Total Days Worked On Project Total: 21
Combined Total Hours Worked: 46.5
...
NEXT MISSION: COMPLETE THE MISSION!

If time:
- Fix pirate bug
- Format better and add more comments
...
for index, row in orioles_batting.iterrows():
    if row["h"] >= 160:
        orioles_batting.at[index, "Point_Category_1"] = 2
    if row["double"] >= 35:
...
# Initializing dataframes for team_a and team_b

team_a = pd.DataFrame({'sum_total_points': [1]})
team_b = pd.DataFrame({'sum_total_points': [1]})
...
# Defining the AL teams and team names

variables_al = [baltimore_orioles, boston_red_sox, new_york_yankees, 
             tampa_bay_rays, toronto_blue_jays, chicago_white_sox,
             cleveland_guardians, detroit_tigers, kansas_city_royals,
             minnesota_twins, houston_astros, los_angeles_angels,
             oakland_athletics, seattle_mariners, texas_rangers]

variable_names_al = ["baltimore_orioles", "boston_red_sox", "new_york_yankees", 
             "tampa_bay_rays", "toronto_blue_jays", "chicago_white_sox",
             "cleveland_guardians", "detroit_tigers", "kansas_city_royals",
             "minnesota_twins", "houston_astros", "los_angeles_angels",
             "oakland_athletics", "seattle_mariners", "texas_rangers"]
...
# Create a DataFrame
al_data = {'Variable': variable_names_al, 'Points': variables_al}
al_df = pd.DataFrame(al_data)

# Rank the variables based on points
al_df['Rank'] = al_df['Points'].rank(method='min', ascending=False)

# Sort the DataFrame by Rank
al_df = al_df.sort_values('Rank')
...
# Defining the NL teams and team names

variables_nl = [atlanta_braves, miami_marlins, new_york_mets,
             washington_nationals, philidelphia_phillies, chicago_cubs,
             cincinatti_reds, milwaukee_brewers, pittsburg_pirates,
             st_louis_cardinals, arizona_diamondbacks, colorado_rockies,
             los_angeles_dodgers, san_diego_padres, san_francisco_giants]

variable_names_nl = ["atlanta_braves", "miami_marlins", "new_york_mets",
             "washington_nationals", "philidelphia_phillies", "chicago_cubs",
             "cincinatti_reds", "milwaukee_brewers", "pittsburg_pirates",
             "st_louis_cardinals", "arizona_diamondbacks", "colorado_rockies",
             "los_angeles_dodgers", "san_diego_padres", "san_francisco_giants"]
...
# Create a DataFrame
nl_data = {'Variable': variable_names_nl, 'Points': variables_nl}
nl_df = pd.DataFrame(nl_data)

# Rank the variables based on points
nl_df['Rank'] = nl_df['Points'].rank(method='min', ascending=False)

# Sort the DataFrame by Rank
nl_df = nl_df.sort_values('Rank')
...
# TOURNEY STYLE 1: THE CLASSIC (structured like how the OG MLB playoffs are,
# every team has a shot to win it all!)
...
### 2 seed versus 15 seed 

al_2seed = al_df.iloc[1]   
al_15seed = al_df.iloc[-1]   

team_a_points = al_2seed['Points']
team_b_points = al_15seed['Points']

winner_2_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_2_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_2_15 == "team_a":
    winner_2_15_data = al_2seed
    print(al_2seed)
else:
    winner_2_15_data = al_15seed
    print(al_15seed)

print(winner_2_15)

### 3 seed versus 14 seed 

al_3seed = al_df.iloc[2]   
al_14seed = al_df.iloc[-2]   

team_a_points = al_3seed['Points']
team_b_points = al_14seed['Points']

winner_3_14, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_3_14}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_3_14 == "team_a":
    winner_3_14_data = al_3seed
    print(al_3seed)
else:
    winner_3_14_data = al_14seed
    print(al_14seed)


### 4 seed versus 13 seed 

al_4seed = al_df.iloc[3]   
al_13seed = al_df.iloc[-3]   

team_a_points = al_4seed['Points']
team_b_points = al_13seed['Points']

winner_4_13, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_4_13}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_4_13 == "team_a":
    winner_4_13_data = al_4seed
    print(al_4seed)
else:
    winner_4_13_data = al_13seed
    print(al_13seed)

### 5 seed versus 12 seed 

al_5seed = al_df.iloc[4]   
al_12seed = al_df.iloc[-4]   

team_a_points = al_5seed['Points']
team_b_points = al_12seed['Points']

winner_5_12, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_5_12}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_5_12 == "team_a":
    winner_5_12_data = al_5seed
    print(al_5seed)
else:
    winner_5_12_data = al_12seed
    print(al_12seed)

### 6 seed versus 11 seed 

al_6seed = al_df.iloc[5]   
al_11seed = al_df.iloc[-5]   

team_a_points = al_6seed['Points']
team_b_points = al_11seed['Points']

winner_6_11, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_6_11}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_6_11 == "team_a":
    winner_6_11_data = al_6seed
    print(al_6seed)
else:
    winner_6_11_data = al_11seed
    print(al_11seed)

### 7 seed versus 10 seed 

al_7seed = al_df.iloc[6]   
al_10seed = al_df.iloc[-6]   

team_a_points = al_7seed['Points']
team_b_points = al_10seed['Points']

winner_7_10, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_7_10}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_7_10 == "team_a":
    winner_7_10_data = al_7seed
    print(al_7seed)
else:
    winner_7_10_data = al_10seed
    print(al_10seed)

### 8 seed versus 9 seed 

al_8seed = al_df.iloc[7]   
al_9seed = al_df.iloc[-7]   

team_a_points = al_8seed['Points']
team_b_points = al_9seed['Points']

winner_8_9, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_8_9}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_8_9 == "team_a":
    winner_8_9_data = al_8seed
    print(al_8seed)
else:
    winner_8_9_data = al_9seed
    print(al_9seed)
...
### 1 seed and winner of 8 and 9 seed
al_1seed = al_df.iloc[0]
team_a_points = al_1seed["Points"]
team_b_points = winner_8_9_data["Points"]

winner_1___8_9, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_1___8_9}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if winner_1___8_9 == "team_a":
    winner_1___8_9_data = al_1seed
    print(al_1seed)
elif winner_8_9 == "team_a":
    winner_1___8_9_data = al_8seed
    print(al_8seed)
elif winner_8_9 == "team_b":
    winner_1___8_9_data = al_9seed
    print(al_9seed)
else:
...
### winner of 2 and 15 seed versus winner of 7 and 10 seed

team_a_points = winner_2_15_data['Points']
team_b_points = winner_7_10_data['Points']

winner_2_15___7_10, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_2_15___7_10}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if winner_2_15 == "team_a" and winner_2_15___7_10 == "team_a":
    winner_2_15___7_10_data = al_2seed
    print(al_2seed)
elif winner_2_15 == "team_b" and winner_2_15___7_10 == "team_a":
    winner_2_15___7_10_data = al_15seed
    print(al_15seed)
elif winner_7_10 == "team_a" and winner_2_15___7_10 == "team_b":
    winner_2_15___7_10_data = al_7seed
    print(al_7seed)
elif winner_7_10 == "team_b" and winner_2_15___7_10 == "team_b":
    winner_2_15___7_10_data = al_10seed
    print(al_10seed)
else:
    print("Something went wrong along the way.")
...
### winner of 3 and 14 seed versus winner of 6 and 11 seed

team_a_points = winner_3_14_data['Points']
team_b_points = winner_6_11_data['Points']

winner_3_14___6_11, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_3_14___6_11}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if winner_3_14 == "team_a" and winner_3_14___6_11 == "team_a":
    winner_3_14___6_11_data = al_3seed
    print(al_3seed)
elif winner_3_14 == "team_b" and winner_3_14___6_11 == "team_a":
    winner_3_14___6_11_data = al_14seed
    print(al_14seed)
elif winner_6_11 == "team_a" and winner_3_14___6_11 == "team_b":
    winner_3_14___6_11_data = al_6seed
    print(al_6seed)
elif winner_6_11 == "team_b" and winner_3_14___6_11 == "team_b":
    winner_3_14___6_11_data = al_11seed
    print(al_11seed)
else:
    print("Something went wrong along the way.")
...
### winner of 4 and 13 seed versus winner of 5 and 12 seed

team_a_points = winner_4_13_data['Points']
team_b_points = winner_5_12_data['Points']

winner_4_13___5_12, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_4_13___5_12}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if winner_4_13 == "team_a" and winner_4_13___5_12 == "team_a":
    winner_4_13___5_12_data = al_4seed
    print(al_4seed)
elif winner_4_13 == "team_b" and winner_4_13___5_12 == "team_a":
    winner_4_13___5_12_data = al_13seed
    print(al_13seed)
elif winner_5_12 == "team_a" and winner_4_13___5_12 == "team_b":
    winner_4_13___5_12_data = al_5seed
    print(al_5seed)
elif winner_5_12 == "team_b" and winner_4_13___5_12 == "team_b":
    winner_4_13___5_12_data = al_12seed
    print(al_12seed)
else:
    print("Something went wrong along the way.")
...
### winner of 1, 4, 5, 8, 9, 12, and 13 seeds

team_a_points = winner_1___8_9_data['Points']
team_b_points = winner_4_13___5_12_data['Points']

winner_1___8_9___4_13___5_12, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_1___8_9___4_13___5_12}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if winner_1___8_9 == "team_a" and winner_1___8_9___4_13___5_12 == "team_a":
    winner_1___8_9___4_13___5_12_data = al_1seed
    print(al_1seed)

elif winner_8_9 == "team_a" and winner_1___8_9 == "team_b" and winner_1___8_9___4_13___5_12 == "team_a":
    winner_1___8_9___4_13___5_12_data = al_8seed
    print(al_8seed)
elif winner_8_9 == "team_b" and winner_1___8_9 == "team_b" and winner_1___8_9___4_13___5_12 == "team_a":
    winner_1___8_9___4_13___5_12_data = al_9seed
    print(al_9seed)

elif winner_4_13 == "team_a" and winner_4_13___5_12 == "team_a" and winner_1___8_9___4_13___5_12 == "team_b":
    winner_1___8_9___4_13___5_12_data = al_4seed
    print(al_4seed)
elif winner_4_13 == "team_b" and winner_4_13___5_12 == "team_a" and winner_1___8_9___4_13___5_12 == "team_b":
    winner_1___8_9___4_13___5_12_data = al_13seed
    print(al_13seed)

elif winner_5_12 == "team_a" and winner_4_13___5_12 == "team_b" and winner_1___8_9___4_13___5_12 == "team_b":
    winner_1___8_9___4_13___5_12_data = al_5seed
    print(al_5seed)
elif winner_5_12 == "team_b" and winner_4_13___5_12 == "team_b" and winner_1___8_9___4_13___5_12 == "team_b":
    winner_1___8_9___4_13___5_12_data = al_12seed
    print(al_12seed)
...
### winner of 2, 3, 6, 7, 10, 11, 14, and 15 seeds

team_a_points = winner_2_15___7_10_data['Points']
team_b_points = winner_3_14___6_11_data['Points']

winner_2_15___3_14___6_11___7_10, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_2_15___3_14___6_11___7_10}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if winner_2_15 == "team_a" and winner_2_15___7_10 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_a":
    winner_2_15___3_14___6_11___7_10_data = al_2seed
    print(al_2seed)
elif winner_2_15 == "team_b" and winner_2_15___7_10 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_a":
    winner_2_15___3_14___6_11___7_10_data = al_15seed
    print(al_15seed)

elif winner_3_14 == "team_a" and winner_3_14___6_11 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_b":
    winner_2_15___3_14___6_11___7_10_data = al_3seed
    print(al_3seed)
elif winner_3_14 == "team_b" and winner_3_14___6_11 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_b":
    winner_2_15___3_14___6_11___7_10_data = al_14seed
    print(al_14seed)

elif winner_6_11 == "team_a" and winner_3_14___6_11 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_b":
    winner_2_15___3_14___6_11___7_10_data = al_6seed
    print(al_6seed)
elif winner_6_11 == "team_b" and winner_3_14___6_11 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_b":
    winner_2_15___3_14___6_11___7_10_data = al_11seed
    print(al_11seed)

elif winner_7_10 == "team_a" and winner_2_15___7_10 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_a":
    winner_2_15___3_14___6_11___7_10_data = al_7seed
    print(al_7seed)
elif winner_7_10 == "team_b" and winner_2_15___7_10 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_a":
    winner_2_15___3_14___6_11___7_10_data = al_10seed
    print(al_10seed)
...
team_a_points = winner_1___8_9___4_13___5_12_data["Points"]
team_b_points = winner_2_15___3_14___6_11___7_10_data['Points']

winner_alcs, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {winner_alcs}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if winner_1___8_9 == "team_a" and winner_1___8_9___4_13___5_12 == "team_a" and winner_alcs == "team_a":
    winner_alcs_data = al_1seed
    print(al_1seed)

elif winner_8_9 == "team_a" and winner_1___8_9 == "team_b" and winner_1___8_9___4_13___5_12 == "team_a" and winner_alcs == "team_a":
    winner_alcs_data = al_8seed
    print(al_8seed)
elif winner_8_9 == "team_b" and winner_1___8_9 == "team_b" and winner_1___8_9___4_13___5_12 == "team_a" and winner_alcs == "team_a":
    winner_alcs_data = al_9seed
    print(al_9seed)

elif winner_4_13 == "team_a" and winner_4_13___5_12 == "team_a" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a":
    winner_alcs_data = al_4seed
    print(al_4seed)
elif winner_4_13 == "team_b" and winner_4_13___5_12 == "team_a" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a":
    winner_alcs_data = al_13seed
    print(al_13seed)

elif winner_5_12 == "team_a" and winner_4_13___5_12 == "team_b" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a":
    winner_alcs_data = al_5seed
    print(al_5seed)
elif winner_5_12 == "team_b " and winner_4_13___5_12 == "team_b" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a":
    winner_alcs_data = al_12seed
    print(al_12seed)

elif winner_2_15 == "team_a" and winner_2_15___7_10 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b":
    winner_alcs_data = al_2seed
    print(al_2seed)
elif winner_2_15 == "team_b" and winner_2_15___7_10 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b":
    winner_alcs_data = al_15seed
    print(al_15seed)

elif winner_3_14 == "team_a" and winner_3_14___6_11 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b":
    winner_alcs_data = al_3seed
    print(al_3seed)
elif winner_3_14 == "team_b" and winner_3_14___6_11 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b":
    winner_alcs_data = al_14seed
    print(al_14seed)

elif winner_6_11 == "team_a" and winner_3_14___6_11 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b":
    winner_alcs_data = al_6seed
    print(al_6seed)
elif winner_6_11 == "team_b" and winner_3_14___6_11 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b":
    winner_alcs_data = al_11seed
    print(al_11seed)

elif winner_7_10 == "team_a" and winner_2_15___7_10 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b":
    winner_alcs_data = al_7seed
    print(al_7seed)
elif winner_7_10 == "team_b" and winner_2_15___7_10 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b":
    winner_alcs_data = al_10seed
    print(al_10seed)
...
nl_2seed = nl_df.iloc[1]   
nl_15seed = nl_df.iloc[-1]   

team_a_points = nl_2seed['Points']
team_b_points = nl_15seed['Points']

nl_winner_2_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_2_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_2_15 == "team_a":
    nl_winner_2_15_data = nl_2seed
    print(nl_2seed)
else:
    nl_winner_2_15_data = nl_15seed
    print(nl_15seed)

print(nl_winner_2_15)

### 3 seed versus 14 seed 

nl_3seed = nl_df.iloc[2]   
nl_14seed = nl_df.iloc[-2]   

team_a_points = nl_3seed['Points']
team_b_points = nl_14seed['Points']

nl_winner_3_14, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_3_14}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_3_14 == "team_a":
    nl_winner_3_14_data = nl_3seed
    print(nl_3seed)
else:
    nl_winner_3_14_data = nl_14seed
    print(nl_14seed)


### 4 seed versus 13 seed 

nl_4seed = nl_df.iloc[3]   
nl_13seed = nl_df.iloc[-3]   

team_a_points = nl_4seed['Points']
team_b_points = nl_13seed['Points']

nl_winner_4_13, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_4_13}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_4_13 == "team_a":
    nl_winner_4_13_data = nl_4seed
    print(nl_4seed)
else:
    nl_winner_4_13_data = nl_13seed
    print(nl_13seed)

### 5 seed versus 12 seed 

nl_5seed = nl_df.iloc[4]   
nl_12seed = nl_df.iloc[-4]   

team_a_points = nl_5seed['Points']
team_b_points = nl_12seed['Points']

nl_winner_5_12, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_5_12}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_5_12 == "team_a":
    nl_winner_5_12_data = nl_5seed
    print(nl_5seed)
else:
    nl_winner_5_12_data = nl_12seed
    print(nl_12seed)

### 6 seed versus 11 seed 

nl_6seed = nl_df.iloc[5]   
nl_11seed = nl_df.iloc[-5]   

team_a_points = nl_6seed['Points']
team_b_points = nl_11seed['Points']

nl_winner_6_11, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_6_11}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_6_11 == "team_a":
    nl_winner_6_11_data = nl_6seed
    print(nl_6seed)
else:
    nl_winner_6_11_data = nl_11seed
    print(nl_11seed)

### 7 seed versus 10 seed 

nl_7seed = nl_df.iloc[6]   
nl_10seed = nl_df.iloc[-6]   

team_a_points = nl_7seed['Points']
team_b_points = nl_10seed['Points']

nl_winner_7_10, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_7_10}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_7_10 == "team_a":
    nl_winner_7_10_data = nl_7seed
    print(nl_7seed)
else:
    nl_winner_7_10_data = nl_10seed
    print(nl_10seed)

### 8 seed versus 9 seed 

nl_8seed = nl_df.iloc[7]   
nl_9seed = nl_df.iloc[-7]   

team_a_points = nl_8seed['Points']
team_b_points = nl_9seed['Points']

nl_winner_8_9, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_8_9}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_8_9 == "team_a":
    nl_winner_8_9_data = nl_8seed
    print(nl_8seed)
else:
    nl_winner_8_9_data = nl_9seed
    print(nl_9seed)
...
### 1 seed and winner of 8 and 9 seed
nl_1seed = nl_df.iloc[0]
team_a_points = nl_1seed["Points"]
team_b_points = nl_winner_8_9_data["Points"]

nl_winner_1___8_9, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_1___8_9}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if nl_winner_1___8_9 == "team_a":
    nl_winner_1___8_9_data = nl_1seed
    print(nl_1seed)
elif nl_winner_8_9 == "team_a":
    nl_winner_1___8_9_data = nl_8seed
    print(nl_8seed)
elif nl_winner_8_9 == "team_b":
    nl_winner_1___8_9_data = nl_9seed
    print(nl_9seed)
else:
    print("Something went wrong along the way.")
...
team_a_points = nl_winner_2_15_data['Points']
team_b_points = nl_winner_7_10_data['Points']

nl_winner_2_15___7_10, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_2_15___7_10}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if nl_winner_2_15 == "team_a" and nl_winner_2_15___7_10 == "team_a":
    nl_winner_2_15___7_10_data = nl_2seed
    print(nl_2seed)
elif nl_winner_2_15 == "team_b" and nl_winner_2_15___7_10 == "team_a":
    nl_winner_2_15___7_10_data = nl_15seed
    print(nl_15seed)
elif nl_winner_7_10 == "team_a" and nl_winner_2_15___7_10 == "team_b":
    nl_winner_2_15___7_10_data = nl_7seed
    print(nl_7seed)
elif nl_winner_7_10 == "team_b" and nl_winner_2_15___7_10 == "team_b":
    nl_winner_2_15___7_10_data = nl_10seed
    print(nl_10seed)
else:
    print("Something went wrong along the way.")
...
team_a_points = nl_winner_3_14_data['Points']
team_b_points = nl_winner_6_11_data['Points']

nl_winner_3_14___6_11, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_3_14___6_11}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if nl_winner_3_14 == "team_a" and nl_winner_3_14___6_11 == "team_a":
    nl_winner_3_14___6_11_data = nl_3seed
    print(nl_3seed)
elif nl_winner_3_14 == "team_b" and nl_winner_3_14___6_11 == "team_a":
    nl_winner_3_14___6_11_data = nl_14seed
    print(nl_14seed)
elif nl_winner_6_11 == "team_a" and nl_winner_3_14___6_11 == "team_b":
    nl_winner_3_14___6_11_data = nl_6seed
    print(nl_6seed)
elif nl_winner_6_11 == "team_b" and nl_winner_3_14___6_11 == "team_b":
    nl_winner_3_14___6_11_data = nl_11seed
    print(nl_11seed)
else:
    print("Something went wrong along the way.")
...
team_a_points = nl_winner_4_13_data['Points']
team_b_points = nl_winner_5_12_data['Points']

nl_winner_4_13___5_12, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_4_13___5_12}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if nl_winner_4_13 == "team_a" and nl_winner_4_13___5_12 == "team_a":
    nl_winner_4_13___5_12_data = nl_4seed
    print(nl_4seed)
elif nl_winner_4_13 == "team_b" and nl_winner_4_13___5_12 == "team_a":
    nl_winner_4_13___5_12_data = nl_13seed
    print(nl_13seed)
elif nl_winner_5_12 == "team_a" and nl_winner_4_13___5_12 == "team_b":
    nl_winner_4_13___5_12_data = nl_5seed
    print(nl_5seed)
elif nl_winner_5_12 == "team_b" and nl_winner_4_13___5_12 == "team_b":
    nl_winner_4_13___5_12_data = nl_12seed
    print(nl_12seed)
else:
    print("Something went wrong along the way.")
...
team_a_points = nl_winner_1___8_9_data['Points']
team_b_points = nl_winner_4_13___5_12_data['Points']

nl_winner_1___8_9___4_13___5_12, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_1___8_9___4_13___5_12}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if nl_winner_1___8_9 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_a":
    nl_winner_1___8_9___4_13___5_12_data = nl_1seed
    print(nl_1seed)

elif nl_winner_8_9 == "team_a" and nl_winner_1___8_9 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_a":
    nl_winner_1___8_9___4_13___5_12_data = nl_8seed
    print(nl_8seed)
elif nl_winner_8_9 == "team_b" and nl_winner_1___8_9 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_a":
    nl_winner_1___8_9___4_13___5_12_data = nl_9seed
    print(nl_9seed)

elif nl_winner_4_13 == "team_a" and nl_winner_4_13___5_12 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_b":
    nl_winner_1___8_9___4_13___5_12_data = nl_4seed
    print(nl_4seed)
elif nl_winner_4_13 == "team_b" and nl_winner_4_13___5_12 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_b":
    nl_winner_1___8_9___4_13___5_12_data = nl_13seed
    print(nl_13seed)

elif nl_winner_5_12 == "team_a" and nl_winner_4_13___5_12 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_b":
    nl_winner_1___8_9___4_13___5_12_data = nl_5seed
    print(nl_5seed)
elif nl_winner_5_12 == "team_b" and nl_winner_4_13___5_12 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_b":
    nl_winner_1___8_9___4_13___5_12_data = nl_12seed
    print(nl_12seed)
...
team_a_points = nl_winner_2_15___7_10_data['Points']
team_b_points = nl_winner_3_14___6_11_data['Points']

nl_winner_2_15___3_14___6_11___7_10, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_2_15___3_14___6_11___7_10}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")


if nl_winner_2_15 == "team_a" and nl_winner_2_15___7_10 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_a":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_2seed
    print(nl_2seed)
elif nl_winner_2_15 == "team_b" and nl_winner_2_15___7_10 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_a":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_15seed
    print(nl_15seed)

elif nl_winner_3_14 == "team_a" and nl_winner_3_14___6_11 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_b":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_3seed
    print(nl_3seed)
elif nl_winner_3_14 == "team_b" and nl_winner_3_14___6_11 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_b":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_14seed
    print(nl_14seed)

elif nl_winner_6_11 == "team_a" and nl_winner_3_14___6_11 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_b":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_6seed
    print(nl_6seed)
elif nl_winner_6_11 == "team_b" and nl_winner_3_14___6_11 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_b":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_11seed
    print(nl_11seed)

elif nl_winner_7_10 == "team_a" and nl_winner_2_15___7_10 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_a":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_7seed
    print(nl_7seed)
elif nl_winner_7_10 == "team_b" and nl_winner_2_15___7_10 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_a":
    nl_winner_2_15___3_14___6_11___7_10_data = nl_10seed
    print(nl_10seed)
...
team_a_points = nl_winner_1___8_9___4_13___5_12_data["Points"]
team_b_points = nl_winner_2_15___3_14___6_11___7_10_data['Points']

nl_winner_nlcs, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_nlcs}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_1___8_9 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_a" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_1seed
    print(nl_1seed)

elif nl_winner_8_9 == "team_a" and nl_winner_1___8_9 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_a" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_8seed
    print(nl_8seed)
elif nl_winner_8_9 == "team_b" and nl_winner_1___8_9 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_a" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_9seed
    print(nl_9seed)

elif nl_winner_4_13 == "team_a" and nl_winner_4_13___5_12 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_4seed
    print(nl_4seed)
elif nl_winner_4_13 == "team_b" and nl_winner_4_13___5_12 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_13seed
    print(nl_13seed)

elif nl_winner_5_12 == "team_a" and nl_winner_4_13___5_12 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_5seed
    print(nl_5seed)
elif nl_winner_5_12 == "team_b " and nl_winner_4_13___5_12 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a":
    nl_winner_nlcs_data = nl_12seed
    print(nl_12seed)

elif nl_winner_2_15 == "team_a" and nl_winner_2_15___7_10 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_2seed
    print(nl_2seed)
elif nl_winner_2_15 == "team_b" and nl_winner_2_15___7_10 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_15seed
    print(nl_15seed)

elif nl_winner_3_14 == "team_a" and nl_winner_3_14___6_11 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_3seed
    print(nl_3seed)
elif nl_winner_3_14 == "team_b" and nl_winner_3_14___6_11 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_14seed
    print(nl_14seed)

elif nl_winner_6_11 == "team_a" and nl_winner_3_14___6_11 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_6seed
    print(nl_6seed)
elif nl_winner_6_11 == "team_b" and nl_winner_3_14___6_11 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_11seed
    print(nl_11seed)

elif nl_winner_7_10 == "team_a" and nl_winner_2_15___7_10 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_7seed
    print(nl_7seed)
elif nl_winner_7_10 == "team_b" and nl_winner_2_15___7_10 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b":
    nl_winner_nlcs_data = nl_10seed
    print(nl_10seed)

else:
    print("Something went wrong along the way.")

print(winner_alcs_data)
print(nl_winner_nlcs_data)
...
team_a_points = winner_alcs_data["Points"]
team_b_points = nl_winner_nlcs_data['Points']

world_series_champs, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {world_series_champs}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")



if winner_1___8_9 == "team_a" and winner_1___8_9___4_13___5_12 == "team_a" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_1seed
    print(al_1seed)

elif winner_8_9 == "team_a" and winner_1___8_9 == "team_b" and winner_1___8_9___4_13___5_12 == "team_a" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_8seed
    print(al_8seed)
elif winner_8_9 == "team_b" and winner_1___8_9 == "team_b" and winner_1___8_9___4_13___5_12 == "team_a" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_9seed
    print(al_9seed)

elif winner_4_13 == "team_a" and winner_4_13___5_12 == "team_a" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_4seed
    print(al_4seed)
elif winner_4_13 == "team_b" and winner_4_13___5_12 == "team_a" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_13seed
    print(al_13seed)

elif winner_5_12 == "team_a" and winner_4_13___5_12 == "team_b" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_5seed
    print(al_5seed)
elif winner_5_12 == "team_b " and winner_4_13___5_12 == "team_b" and winner_1___8_9___4_13___5_12 == "team_b" and winner_alcs == "team_a" and world_series_champs == "team_a":
    world_series_champs_data = al_12seed
    print(al_12seed)

elif winner_2_15 == "team_a" and winner_2_15___7_10 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_2seed
    print(al_2seed)
elif winner_2_15 == "team_b" and winner_2_15___7_10 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_15seed
    print(al_15seed)

elif winner_3_14 == "team_a" and winner_3_14___6_11 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_3seed
    print(al_3seed)
elif winner_3_14 == "team_b" and winner_3_14___6_11 == "team_a" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_14seed
    print(al_14seed)

elif winner_6_11 == "team_a" and winner_3_14___6_11 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_6seed
    print(al_6seed)
elif winner_6_11 == "team_b" and winner_3_14___6_11 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_b" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_11seed
    print(al_11seed)

elif winner_7_10 == "team_a" and winner_2_15___7_10 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_7seed
    print(al_7seed)
elif winner_7_10 == "team_b" and winner_2_15___7_10 == "team_b" and winner_2_15___3_14___6_11___7_10 == "team_a" and winner_alcs == "team_b" and world_series_champs == "team_a":
    world_series_champs_data = al_10seed
    print(al_10seed)

elif nl_winner_1___8_9 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_a" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_1seed
    print(nl_1seed)

elif nl_winner_8_9 == "team_a" and nl_winner_1___8_9 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_a" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_8seed
    print(nl_8seed)
elif nl_winner_8_9 == "team_b" and nl_winner_1___8_9 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_a" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_9seed
    print(nl_9seed)

elif nl_winner_4_13 == "team_a" and nl_winner_4_13___5_12 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_4seed
    print(nl_4seed)
elif nl_winner_4_13 == "team_b" and nl_winner_4_13___5_12 == "team_a" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_13seed
    print(nl_13seed)

elif nl_winner_5_12 == "team_a" and nl_winner_4_13___5_12 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_5seed
    print(nl_5seed)
elif nl_winner_5_12 == "team_b " and nl_winner_4_13___5_12 == "team_b" and nl_winner_1___8_9___4_13___5_12 == "team_b" and nl_winner_nlcs == "team_a" and world_series_champs == "team_b":
    world_series_champs_data = nl_12seed
    print(nl_12seed)

elif nl_winner_2_15 == "team_a" and nl_winner_2_15___7_10 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_2seed
    print(nl_2seed)
elif nl_winner_2_15 == "team_b" and nl_winner_2_15___7_10 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_15seed
    print(nl_15seed)

elif nl_winner_3_14 == "team_a" and nl_winner_3_14___6_11 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_3seed
    print(nl_3seed)
elif nl_winner_3_14 == "team_b" and nl_winner_3_14___6_11 == "team_a" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_14seed
    print(nl_14seed)

elif nl_winner_6_11 == "team_a" and nl_winner_3_14___6_11 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_6seed
    print(nl_6seed)
elif nl_winner_6_11 == "team_b" and nl_winner_3_14___6_11 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_b" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_11seed
    print(nl_11seed)

elif nl_winner_7_10 == "team_a" and nl_winner_2_15___7_10 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_7seed
    print(nl_7seed)
elif nl_winner_7_10 == "team_b" and nl_winner_2_15___7_10 == "team_b" and nl_winner_2_15___3_14___6_11___7_10 == "team_a" and nl_winner_nlcs == "team_b" and world_series_champs == "team_b":
    world_series_champs_data = nl_10seed
    print(nl_10seed)
...
# Importing re package in order to find the champions name from within the data
import re


# Initializing the series
series = pd.Series(world_series_champs_data)

# Convert the Series to a string
series_str = series.to_string()

# Defining the regex pattern to find parts with at least one underscore
pattern = r'\b\w+_\w+\b'

# Using regex to find all matches
matches = re.findall(pattern, series_str)

# Printing the matches by getting indexing the lone value from the list
if matches:
    champion = matches[0]  # This will get the first match
else:
    print("Error")

print(f"Congratulations to the {champion} for winning the world series!")



# TOURNEY STYLE 2: THE GAUNTLET (much harder for the lower seeds to win
# because we are seeking out the best team of all time)
...
al_7seed = al_df.iloc[6]
al_8seed = al_df.iloc[7]
al_9seed = al_df.iloc[8]
al_10seed = al_df.iloc[9]
al_11seed = al_df.iloc[10]
al_12seed = al_df.iloc[11]
al_13seed = al_df.iloc[12]
al_14seed = al_df.iloc[13]
al_15seed = al_df.iloc[14]
...
nl_7seed = nl_df.iloc[6]
nl_8seed = nl_df.iloc[7]
nl_9seed = nl_df.iloc[8]
nl_10seed = nl_df.iloc[9]
nl_11seed = nl_df.iloc[10]
nl_12seed = nl_df.iloc[11]
nl_13seed = nl_df.iloc[12]
nl_14seed = nl_df.iloc[13]
nl_15seed = nl_df.iloc[14]



# AL

team_a_points = al_14seed['Points']
team_b_points = al_15seed['Points']

al_winner_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_14_15 == "team_a":
    al_winner_14_15_data = al_14seed
    print(al_14seed)
else:
    al_winner_14_15_data = al_15seed
    print(al_15seed)




team_a_points = al_13seed['Points']
team_b_points = al_winner_14_15_data['Points']

al_winner_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_13_14_15 == "team_a":
    al_winner_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_13_14_15_data = al_15seed
    print(al_15seed)




team_a_points = al_12seed['Points']
team_b_points = al_winner_13_14_15_data['Points']

al_winner_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_12_13_14_15 == "team_a":
    al_winner_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_12_13_14_15_data = al_15seed
    print(al_15seed)




team_a_points = al_11seed['Points']
team_b_points = al_winner_12_13_14_15_data['Points']

al_winner_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_11_12_13_14_15 == "team_a":
    al_winner_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_10seed['Points']
team_b_points = al_winner_11_12_13_14_15_data['Points']

al_winner_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)




team_a_points = al_9seed['Points']
team_b_points = al_winner_10_11_12_13_14_15_data['Points']

al_winner_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_8seed['Points']
team_b_points = al_winner_9_10_11_12_13_14_15_data['Points']

al_winner_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_7seed['Points']
team_b_points = al_winner_8_9_10_11_12_13_14_15_data['Points']

al_winner_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_7_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_7_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_6seed['Points']
team_b_points = al_winner_7_8_9_10_11_12_13_14_15_data['Points']

al_winner_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_6seed
    print(al_6seed)

elif al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_6_7_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_5seed['Points']
team_b_points = al_winner_6_7_8_9_10_11_12_13_14_15_data['Points']

al_winner_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_5seed
    print(al_5seed)

elif al_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_6seed
    print(al_6seed)

elif al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_5_6_7_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_4seed['Points']
team_b_points = al_winner_5_6_7_8_9_10_11_12_13_14_15_data['Points']

al_winner_4_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_4_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_4seed
    print(al_4seed)

elif al_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_5seed
    print(al_5seed)

elif al_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_6seed
    print(al_6seed)

elif al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_3seed['Points']
team_b_points = al_winner_4_5_6_7_8_9_10_11_12_13_14_15_data['Points']

al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_3seed
    print(al_3seed)

elif al_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_4seed
    print(al_4seed)

elif al_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_5seed
    print(al_5seed)

elif al_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_6seed
    print(al_6seed)

elif al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_2seed['Points']
team_b_points = al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data['Points']

al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_2seed
    print(al_2seed)

elif al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_3seed
    print(al_3seed)

elif al_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_4seed
    print(al_4seed)

elif al_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_5seed
    print(al_5seed)

elif al_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_6seed
    print(al_6seed)

elif al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_14seed
    print(al_14seed)

else:
    al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = al_15seed
    print(al_15seed)
...
team_a_points = al_1seed['Points']
team_b_points = al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data['Points']

al_champ, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {al_champ}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if al_champ == "team_a":
    al_champ_data = al_1seed
    print(al_1seed)

elif al_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_2seed
    print(al_2seed)

elif al_winner_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_3seed
    print(al_3seed)

elif al_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_4seed
    print(al_4seed)

elif al_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_5seed
    print(al_5seed)

elif al_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_6seed
    print(al_6seed)

elif al_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_7seed
    print(al_7seed)

elif al_winner_8_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_8seed
    print(al_8seed)

elif al_winner_9_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_9seed
    print(al_9seed)

elif al_winner_10_11_12_13_14_15 == "team_a":
    al_champ_data = al_10seed
    print(al_10seed)

elif al_winner_11_12_13_14_15 == "team_a":
    al_champ_data = al_11seed
    print(al_11seed)

elif al_winner_12_13_14_15 == "team_a":
    al_champ_data = al_12seed
    print(al_12seed)

elif al_winner_13_14_15 == "team_a":
    al_champ_data = al_13seed
    print(al_13seed)

elif al_winner_14_15 == "team_a":
    al_champ_data = al_14seed
    print(al_14seed)

else:
    al_champ_data = al_15seed
    print(al_15seed)
...
team_a_points = nl_14seed['Points']
team_b_points = nl_15seed['Points']

nl_winner_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_14_15 == "team_a":
    nl_winner_14_15_data = nl_14seed
    print(nl_14seed)
else:
    nl_winner_14_15_data = nl_15seed
    print(nl_15seed)




team_a_points = nl_13seed['Points']
team_b_points = nl_winner_14_15_data['Points']

nl_winner_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_13_14_15 == "team_a":
    nl_winner_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_13_14_15_data = nl_15seed
    print(nl_15seed)




team_a_points = nl_12seed['Points']
team_b_points = nl_winner_13_14_15_data['Points']

nl_winner_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_12_13_14_15 == "team_a":
    nl_winner_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_12_13_14_15_data = nl_15seed
    print(nl_15seed)




team_a_points = nl_11seed['Points']
team_b_points = nl_winner_12_13_14_15_data['Points']

nl_winner_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_10seed['Points']
team_b_points = nl_winner_11_12_13_14_15_data['Points']

nl_winner_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)




team_a_points = nl_9seed['Points']
team_b_points = nl_winner_10_11_12_13_14_15_data['Points']

nl_winner_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_8seed['Points']
team_b_points = nl_winner_9_10_11_12_13_14_15_data['Points']

nl_winner_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_7seed['Points']
team_b_points = nl_winner_8_9_10_11_12_13_14_15_data['Points']

nl_winner_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_7_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_6seed['Points']
team_b_points = nl_winner_7_8_9_10_11_12_13_14_15_data['Points']

nl_winner_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_6seed
    print(nl_6seed)

elif nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_6_7_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_5seed['Points']
team_b_points = nl_winner_6_7_8_9_10_11_12_13_14_15_data['Points']

nl_winner_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_5seed
    print(nl_5seed)

elif nl_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_6seed
    print(nl_6seed)

elif nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_5_6_7_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_4seed['Points']
team_b_points = nl_winner_5_6_7_8_9_10_11_12_13_14_15_data['Points']

nl_winner_4_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_4_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_4seed
    print(nl_4seed)

elif nl_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_5seed
    print(nl_5seed)

elif nl_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_6seed
    print(nl_6seed)

elif nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_3seed['Points']
team_b_points = nl_winner_4_5_6_7_8_9_10_11_12_13_14_15_data['Points']

nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_3seed
    print(nl_3seed)

elif nl_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_4seed
    print(nl_4seed)

elif nl_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_5seed
    print(nl_5seed)

elif nl_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_6seed
    print(nl_6seed)

elif nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_2seed['Points']
team_b_points = nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15_data['Points']

nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_2seed
    print(nl_2seed)

elif nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_3seed
    print(nl_3seed)

elif nl_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_4seed
    print(nl_4seed)

elif nl_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_5seed
    print(nl_5seed)

elif nl_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_6seed
    print(nl_6seed)

elif nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_14seed
    print(nl_14seed)

else:
    nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data = nl_15seed
    print(nl_15seed)
...
team_a_points = nl_1seed['Points']
team_b_points = nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15_data['Points']

nl_champ, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {nl_champ}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")

if nl_champ == "team_a":
    nl_champ_data = nl_1seed
    print(nl_1seed)

elif nl_winner_2_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_2seed
    print(nl_2seed)

elif nl_winner_3_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_3seed
    print(nl_3seed)

elif nl_winner_4_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_4seed
    print(nl_4seed)

elif nl_winner_5_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_5seed
    print(nl_5seed)

elif nl_winner_6_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_6seed
    print(nl_6seed)

elif nl_winner_7_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_7seed
    print(nl_7seed)

elif nl_winner_8_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_8seed
    print(nl_8seed)

elif nl_winner_9_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_9seed
    print(nl_9seed)

elif nl_winner_10_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_10seed
    print(nl_10seed)

elif nl_winner_11_12_13_14_15 == "team_a":
    nl_champ_data = nl_11seed
    print(nl_11seed)

elif nl_winner_12_13_14_15 == "team_a":
    nl_champ_data = nl_12seed
    print(nl_12seed)

elif nl_winner_13_14_15 == "team_a":
    nl_champ_data = nl_13seed
    print(nl_13seed)

elif nl_winner_14_15 == "team_a":
    nl_champ_data = nl_14seed
    print(nl_14seed)

else:
    nl_champ_data = nl_15seed
    print(nl_15seed)
...
### BIG NOTE, I SHOULD HAVE DONE IT THIS WAY FROM THE START, copying over the champion data instead of doing if statements for every seed

team_a_points = al_champ_data['Points']
team_b_points = nl_champ_data['Points']

world_champs, team_a_score, team_b_score = determine_winner_and_scores(team_a_points, team_b_points)

# Printing results
print(f"Winner: {world_champs}")
print(f"Team A Score: {team_a_score}")
print(f"Team B Score: {team_b_score}")
...
if world_champs == "team_a":
    champion_data = al_champ_data
    print(champion_data)

else:
    champion_data = nl_champ_data
    print(champion_data)



# Initializing the series
series = pd.Series(champion_data)
